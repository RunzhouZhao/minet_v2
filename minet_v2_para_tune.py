...
total_embed_dim_1 = n_one_hot_slot_1 * k
...
for item in para_list:
    ...
    def partition_input_1(x_input):
        ...
        x_input_user_one_hot = x_input[:, 0:idx_list[0]]
        x_input_user_mul_hot = x_input[:, idx_list[0]:idx_list[1]]
        x_input_user_mul_hot = tf.reshape(x_input_user_mul_hot, [-1, n_user_mul_hot_slot, max_len_per_slot_u])
        # shape=[None, n_one_hot_slot]
        x_input_one_hot = x_input[:, idx_list[1]:idx_list[2]]
        x_input_one_hot_r=x_input[:, idx_list[2]:idx_list[3]]
        x_input_mul_hot_r=x_input[:, idx_list[3]:idx_list[4]]
        x_input_mul_hot_r = tf.reshape(x_input_mul_hot_r, [-1, n_mul_hot_slot_2, max_len_per_slot_2])
        
        #######################
        # tar domain clk
        concat_one_hot_1 = x_input[:, idx_list[4]:idx_list[5]]
        for i in range(1, max_n_clk_ori_1):
            # one_hot
            temp_1 = x_input[:, idx_list[i+4]:idx_list[i+5]]
        ...
        return x_input_user_one_hot, x_input_user_mul_hot, x_input_one_hot, x_input_one_hot_r, x_input_mul_hot_r, x_input_one_hot_1, \
    ...
    def get_concate_embed(x_input_one_hot, total_embed_dim):
        data_embed_one_hot = get_masked_one_hot(x_input_one_hot)
        data_embed_one_hot = tf.reshape(data_embed_one_hot, [-1, total_embed_dim])
        return data_embed_one_hot
    
    def get_concate_embed_c(x_input_one_hot, x_input_mul_hot, total_embed_dim):
        data_embed_one_hot = get_masked_one_hot(x_input_one_hot)
    ...
    def get_user_embed(x_input_user_one_hot, x_input_user_mul_hot, total_embed_dim_user):
        data_embed_one_hot_user = get_masked_one_hot(x_input_user_one_hot)
        data_embed_mul_hot_user = get_masked_mul_hot(x_input_user_mul_hot)
        data_embed_concat = tf.concat([data_embed_one_hot_user, data_embed_mul_hot_user], 1)
        data_embed_concat = tf.reshape(data_embed_concat, [-1, total_embed_dim_user])
        return data_embed_concat
    ...
    def get_concate_embed_clk_a(x_input_one_hot_clk, max_n_clk, total_embed_dim):
        data_embed_one_hot = get_masked_one_hot_clk(x_input_one_hot_clk)
        data_embed_one_hot = tf.reshape(data_embed_one_hot, [-1, max_n_clk, total_embed_dim])
        return data_embed_one_hot
    
    def reshape_data_user_embed(data_embed_1, data_embed_r, data_embed_user, max_n_clk):
        ...
        return data_embed_1_re, data_embed_r_re, data_embed_user_re

    # input: (?, max_n_clk, total_embed_dim)
    # output: (?, total_embed_dim)
    # n_valid: (?, 1)
    def item_level_att_1(data_embed_clk_1, data_embed_1_re, data_embed_r_re, data_embed_user_re, \
                                x_input_one_hot_clk_1):
        # convert to 2D
        data_embed_clk_1_re = tf.reshape(data_embed_clk_1, [-1, total_embed_dim_1])
        cur_input = tf.concat([data_embed_clk_1_re, data_embed_1_re, data_embed_r_re, data_embed_user_re, 
    ...
    def item_level_att_2(data_embed_clk_2, data_embed_1_re, data_embed_r_re, data_embed_user_re, \
    ...
    def interest_level_att(data_embed_1, data_embed_r, data_embed_user_1, data_embed_clk_1_agg, \
                            data_embed_clk_2_agg):
        data_embed_z = tf.concat([data_embed_1, data_embed_r, data_embed_user_1, data_embed_clk_1_agg, \
        ...
        data_embed_dg = tf.concat([data_embed_1, data_embed_r, weight_0*data_embed_user_1, \
    ...
    ##########################################################
    # MiNet
    x_input_user_one_hot, x_input_user_mul_hot, x_input_one_hot, x_input_one_hot_r, x_input_mul_hot_r, x_input_one_hot_1, \
        x_input_one_hot_2, x_input_mul_hot_2 \
    ...
    data_embed_1_re_1, data_embed_r_re_1, data_embed_user_1_re_1 = reshape_data_user_embed(data_embed_1, data_embed_r, data_embed_user_1, max_n_clk_1)
    data_embed_1_re_2, data_embed_r_re_2, data_embed_user_1_re_2 = reshape_data_user_embed(data_embed_1, data_embed_r, data_embed_user_1, max_n_clk_2)
    # item-level att
    data_embed_agg_1_ig = item_level_att_1(data_embed_clk_1, data_embed_1_re_1, data_embed_r_re_1, data_embed_user_1_re_1, \
                        x_input_one_hot_1)
    data_embed_agg_2_ig = item_level_att_2(data_embed_clk_2, data_embed_1_re_2, data_embed_r_re_1, data_embed_user_1_re_2, \
                        x_input_one_hot_2)

    # interest-level att
    data_embed_1_final = interest_level_att(data_embed_1, data_embed_r, data_embed_user_1, data_embed_agg_1_ig,\
                            data_embed_agg_2_ig)
            ...
            while not coord.should_stop(): 
                ...
                if (epoch+1)%record_step_size == 0:
                    train_loss_temp = sess.run(loss, feed_dict={ \
                                               x_input_1:train_ft_inst_1, \
                                               y_target_1:train_label_inst_1, keep_prob:1.0})
