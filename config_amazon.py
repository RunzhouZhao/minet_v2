...
n_one_hot_slot_1 = 19 # only item fts; no user fts; id, main_cate, brand 
# title, categories
max_len_per_slot_1 = 3
num_csv_col_1 = 224 # num of cols in the csv file = 1 (label) + n_user_slot + (n_one_hot_slot_1 + n_mul_hot_slot_1*max_len_per_slot_1)*(1 + max_n_clk_ori_1) + (n_one_hot_slot_2 + n_mul_hot_slot_2*max_len_per_slot_2)*max_n_clk_ori_2
batch_size_1 = 1024
...
n_one_hot_slot_2 = 13 # only item fts; no user fts
n_mul_hot_slot_2 = 6
...
eta_range = [0.0005,0.001,0.01] # [0.01, 0.02, 0.05, 0.1]
inter_dim = 10
...
n_epoch = 10 # number of times to loop over the whole data set
record_step_size = 10 # record the loss and auc after xx mini-batches
